...
class DatasetKVASIR(Dataset):
    ...
    def __init__(self, datapath, fold, transform, split, way, shot, transforms_aug=[]):
        ...
        self.nclass = 8  # Update based on Kvasir's actual number of classes
        self.benchmark = 'kvasir'
        self.CLASSES = ('dyed-lifted-polyps', 'dyed-resection-margins', 'esophagitis',
                        'normal-cecum', 'normal-pylorus', 'normal-z-line', 'polyps',
                        'ulcerative-colitis')  # Update these class names based on Kvasir
        ...
        # self.img_path = os.path.join(datapath, 'VOC2012/JPEGImages/') # take the path to the image directory 
        # self.ann_path = os.path.join(datapath, 'VOC2012/SegmentationClassAug/') # take the path to the segmentation class file  (corresponding image mask)
        self.img_path = os.path.join(datapath, 'Kvasir/kvasir-dataset-v2/JPEGImages/')  # Update path
        self.ann_path = os.path.join(datapath, 'Kvasir/kvasir-dataset-v2/segmentationClass/')  # Update pat
...
#  It resizes the query segmentation mask during training and retains the original size during validation. It then calls the generate_query_episodic_mask method to generate the query mask and the corresponding ignore index.
    ...
    def read_mask(self, img_name):
        r"""Return segmentation mask in PIL Image and with help of numpy library it then converts into tensor"""
    ...
    def build_img_metadata(self):
        ...
        def read_metadata(split, fold_id):
            fold_n_metadata = os.path.join(f'data/splits/kvasir/{split}/fold{fold_id}.txt')
